...
<html>
    ...
    foreach ($zmogus as  $issigalvotasKintamasisStalciausTurinys) {
        echo $issigalvotasKintamasisStalciausTurinys . "<br>";
    }
    print_r($zmogus); // pasitikrinimas - greitas array isvedimas
    var_dump($zmogus); // isveda visa masyva/kint.   ir  jo "type"

    $vardas = "Raudonkepuraite";
    $vieta = "miska";
    $gyvun = "meska";
    printf( "%s ejo pasivaikscioti po %s  ir sutiko %s pikta.",
                $vardas, $vieta, $gyvun);

    
    foreach ($zmogus as $indexKey => $stalcTurinys) {
        echo "stalcius vardas: ". $indexKey . "<br>";
    }
             ?>
