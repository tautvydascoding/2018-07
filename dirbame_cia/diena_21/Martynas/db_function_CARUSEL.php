...
function createCaruselImg($vardas, $article)
{
...
// patikrina ar pavyko prisijungti.
  if ($arPavyko == false) {
    echo " error : nepavyko sukurti naujo caruselIMG : $vardas, $article < br / > " . mysqli_error(getPrisijungimas());
  }
}
...
// nesumaisyti!        queris   prisijungimas     ir manoSQL uzklausa
  ...
  if ($arPavyko == false) {
    echo " error : nepavyko istrinti caruselIMG, kurio nr: $nr < br / > " . mysqli_error(getPrisijungimas());
  }
}
// deleteDoctor(1);
// deletePatient(8);
...
function updateCaruselImg($nr, $vardas, $pavarde, $gydNr)
{
  ...
  if (!$arVeikia) {
    echo " error : nepavyko atnaujinti/pakeisti  caruselIMG, kurio nr: $nr < br / > " . mysqli_error(getPrisijungimas());
  }
}
